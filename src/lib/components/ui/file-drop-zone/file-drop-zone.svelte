<script lang="ts">
  ...
  const drop = async (
    ...
    // If no onUpload handler is provided, we ignore drag-and-drop
    // and rely on the native file input + form submission.
    if (!onUpload) return;
  ...
  const change = async (
    ...
    // If an onUpload handler is provided, we treat this as an
    // immediate-upload component and don't keep the file on the input.
    // If onUpload is not provided, we leave the input value intact so
    // the file is sent with the form submission.
    if (onUpload) {
